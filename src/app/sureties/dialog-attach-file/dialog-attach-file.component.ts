...
export class DialogAttachFileComponent {
    ...
    fetchData() {
        this.suretiesService.getSuretyNodes(this.data.fileType, this.data.suretyId).subscribe(suretyNodes => {
            this.suretyNodes = suretyNodes
        })
    }

    async onSelectNode(suretyNode: SuretyNodeModel) {
        this.suretyNodeId = suretyNode._id
        switch (suretyNode.mimeType) {
            case 'application/pdf': {
                this.tabIndex = 1
                const file = await this.suretiesService.getFile(`suretyNodes/files/${suretyNode._id}`)
            ...
            default: {
                const file = await this.suretiesService.getFile(`suretyNodes/files/${suretyNode._id}`)
                const urlFile = window.URL.createObjectURL(file)
                this.downloadFile(urlFile, suretyNode.name)
                break
            }
        }
    }
    ...
    onDeleteFile() {
        this.tabIndex = 0
        this.suretiesService.deleteFile(this.suretyNodeId).subscribe(() => {
            this.suretyNodeId = ''
            this.fetchData()
        })
    }
    ...
    onFileSelected(files: FileList | null, input: HTMLInputElement) {
        if (files !== null) {
            ...
            for (let index = 0; index < files.length; index++) {
                ...
                this.suretiesService.uploadFile(formData, this.data.fileType, this.data.suretyId).subscribe(suretyNode => {
                    this.fetchData()
                })
                ...
                //         this.suretiesService.uploadFile(formData, this.data.suretyId, this.data.type).subscribe(pdfId => {
                //             console.log(pdfId);
                //             this.fetchData();
                //         });
                //     });
